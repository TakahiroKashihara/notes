...
@everywhere function qmgk(k::Vector{Float64},p::Parm,se::SE)#w積分とw1積分を一気にやってしまう。
    gxxk = 0.
    gyyk = 0.
    gxyk = 0.
...
@everywhere function trgkm(k::Vector{Float64},M::Float64)#行列から計算する量子計量
...
@everywhere function detgkm(k::Vector{Float64},M::Float64)#行列から計算する量子計量
    v,w = eigen(H(k,M))
    pv = v[2]
    mv = v[1]
    pw = w[:,2]
...
function ok()#どこまで実行されたかチェック
    println("ok")
    return()
end
function mainU()#計算部分
    p = Parm(#=retestと合わせる=#0.2, #=Nk=#100, #=Nw.これもreshapeと合わせる=#1000, #=delta=# 1/20/100, #=beta=# 10000, #=\mu=# parse.(Float64,ARGS[2])/2, #=M=# parse.(Float64,ARGS[1]), #=U=# parse.(Float64,ARGS[2]))
    se = SE(swise(p.M,p.U,p)...)
    Nkh = Int(p.Nk/2)#高対称点を通る経路の1辺の長さ
    println("NK = $(p.Nk)")
    ...
    ([1,Int(length(va.hsbz)/3),Int(length(va.hsbz)*2/3),length(va.hsbz)],[L"\Gamma",L"\mathrm{X}",L"\mathrm{M}",L"\Gamma"]),marker=:circle)
    ...
    #比較
    plot(trghs,ylabel = "QuantumMetric",label = L"\mathrm{Tr}(g)_\mathrm{Green}",xticks = 
    ([1,Int(length(va.hsbz)/3),Int(length(va.hsbz)*2/3),length(va.hsbz)],[L"\Gamma",L"\mathrm{X}",L"\mathrm{M}",L"\Gamma"]),marker=:circle)
    plot!(trghsm,ylabel = "QuantumMetric",label = L"\mathrm{Tr}(g)_\mathrm{matrix}",xticks = 
    ([1,Int(length(va.hsbz)/3),Int(length(va.hsbz)*2/3),length(va.hsbz)],[L"\Gamma",L"\mathrm{X}",L"\mathrm{M}",L"\Gamma"]),marker=:circle)
    savefig("./trcompare_hs_M=$(ARGS[1])_U=$(ARGS[2]).svg")
    
    plot(sqdetghs,ylabel = "QuantumMetric",label = L"\sqrt{\mathrm{det}(g)}_\mathrm{Green}",xticks = 
    ([1,Int(length(va.hsbz)/3),Int(length(va.hsbz)*2/3),length(va.hsbz)],[L"\Gamma",L"\mathrm{X}",L"\mathrm{M}",L"\Gamma"]),marker=:circle)
    plot!(sqdetghsm,ylabel = "QuantumMetric",label =  L"\sqrt{\mathrm{det}(g)}_\mathrm{matrix}",xticks = 
    ([1,Int(length(va.hsbz)/3),Int(length(va.hsbz)*2/3),length(va.hsbz)],[L"\Gamma",L"\mathrm{X}",L"\mathrm{M}",L"\Gamma"]),marker=:circle)
...
include("retestserver.jl")#呼び出すSelfEnergyがちゃんとあっているか確認しよう！
